...
import {ConnectComponent} from '../../components/dashboard/connect/connect.component';
...
export class CrudDetailPage implements OnInit, AfterViewInit {
  ...
  constructor(@Inject(LOCALE_ID) private locale: string,
    ...
    this.activatedRoute.queryParams.subscribe(() => {
      if (this.router.getCurrentNavigation().extras.state) {
        this.crudDetail = this.router.getCurrentNavigation().extras.state.crud;
  ...
  favourite() {
    this.apiService.saveFavourite(this.cacheService.user.uid, this.crudDetail).then(() => this.utilService.showToast('Successfully saved!'));
  }
  ...
  async connect() {
    ...
    modal.onDidDismiss().then(overlayEventResult => {
      if (overlayEventResult.data) {
        this.apiService.bookCrud(this.cacheService.user.uid, this.crudDetail, overlayEventResult.data.contactInfo, overlayEventResult.data.contactMessage);
        const notificationData = {
          fcmToken: this.crudDetail.user.fcmToken,
          title: 'Incoming Requests',
          body: 'From ' + this.cacheService.user.username
        };
        this.functionService.pushNotification(notificationData);
      }
    });
